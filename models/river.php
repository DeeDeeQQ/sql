...
function updateRiver($riverId, $riverTitle)
{
    $db = getDbConnection();
    $sql = "UPDATE rivers SET title = ? WHERE id = ?";
    $query = mysqli_prepare($db,$sql);
    mysqli_stmt_bind_param($query,'si',$riverTitle,$riverId);

    return mysqli_stmt_execute($query);
}

function setNewCountry($riverId, $countryTitle)
{
